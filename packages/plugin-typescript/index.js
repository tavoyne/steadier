...
const baseRules = require("@steadier/eslint-plugin-base").configs.all.rules;
...
Object.entries(allTypescriptRules)
  ...
  .forEach(([name, rule]) => {
    ...
    if (baseRules[baseName]) {
      plugin.configs.all.rules[baseName] = "off";
      plugin.configs.all.rules[`@typescript-eslint/${name}`] =
        baseRules[baseName];
    }
  });
